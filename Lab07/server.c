...
int main() {
	...
	// 
	char command[100] = "ls > file.txt";

	int result = system(command);

	if (result == -1)
	{
		// system() failed to execute the command processor
	...
	send_msg.sin_port = htons(5000);     // host byte order to network byte order
	...
	while(1) {
		...
		ssize_t NumberOfBytesSend = send(connecn_fd[0], 
										send_buff, 
										strlen(send_buff), 
										0);
